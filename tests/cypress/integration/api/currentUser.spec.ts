...
describe('Validate ability get current User', () => {
    it('Get Current user for Authenticated user (irina)', () => {
        cy.apolloClient({username:'irina', password: 'password'}).apollo({queryFile: 'currentUser.graphql'}).should((response: any) => {
    ...
    it('Get Current user for Authenticated user (root) with an incorrect password', () => {
        cy.apolloClient({username:'root', password: 'THIS-IS-INCORRECT'}).apollo({queryFile: 'currentUser.graphql', errorPolicy:'all'}).should((response: any) => {
            expect(response.data.currentUser).to.be.null
        })
    })

    it('Get Current user for Authenticated user (root) with an incorrect user', () => {
        cy.apolloClient({username:'I-DO-NOT-EXIST', password: 'THIS-IS-INCORRECT'}).apollo({queryFile: 'currentUser.graphql', errorPolicy:'all'}).should((response: any) => {
            expect(response.data.currentUser).to.be.null
        })
    })
})
